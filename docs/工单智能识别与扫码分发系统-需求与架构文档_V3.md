...
**日期**: 2026-07-23
**状态**: 已通过SRE评审（待开发）
**变更说明**: V2.0 → V3.0 基于生产就绪评审补全高可用/安全/运维/一致性设计
...
| 目标 | 指标 | 阶段 |
|------|------|------|
| 实现工单自动识别入库 | OCR识别准确率 ≥ **95%**（关键字段） | Phase 1 |
...
| M1-03 | 从识别结果中提取结构化字段：工单号、客户料号、产品编码、预计产量、PO号、客户、交货日期、批次数量、下单日期、计划日期 | 关键字段准确率 ≥ **95%** |
...
| M3-02 | 批量生成二维码（最多100个） | 导出PDF供打印，**支持失败重试和进度追踪** |
...
| M5-10 | 操作日志 | 记录所有操作 |
| 🆕M5-11 | 冲突解决UI | 离线数据冲突时弹窗展示差异，用户选择保留方 |
...
| BR-06 | 🆕 **离线报工冲突解决策略**：①客户端提交时携带服务端数据版本号(version)；②服务端比对版本，若一致则直接合并；③若不一致则标记为CONFLICT状态，返回给客户端；④客户端弹出冲突解决UI，展示本地vs服务端差异，由用户选择保留方；⑤冲突记录写入conflict_logs表供管理员审计 |
...
| | 🆕PUT | /api/v1/processes/{id}/report | 工序报工（**幂等**，需X-Idempotency-Key） |
| 二维码 | POST | /api/v1/qrcode/generate | 生成单张 |
| | 🆕POST | /api/v1/qrcode/batch | 批量生成（**异步Saga**，返回taskId） |
...
| | 🆕PUT | /api/v1/app/conflicts/{id}/resolve | 解决冲突 |
...
| **V3.0** | **2026-07-23** | **SRE评审后全面补全：高可用(§18)+数据一致性(§19)+安全加固(§13重写)+监控(§14重写)+资源修正(§15)+异常场景(§21)+压测(§22)+评审记录(§23)** | **SRE架构师** | **✅ 条件通过（待压测验证）** |
